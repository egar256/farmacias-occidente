...
function Turnos() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await updateTurno(editingId, formData);
      ...
      alert('Error al guardar turno: ' + (error.response?.data?.error || error.message));
    }
  };
        ...
        {turnos.length === 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {turnos.map((turno) => (
                <tr key={turno.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{turno.nombre}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-center">{turno.orden}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${turno.activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
